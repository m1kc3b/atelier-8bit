# CHUCK-8 **Computer System Specification  —  v1.1
## Version 1.1 — Corrigée & Révisée*
...
*Modifications v1.0 → v1.1 : 28 corrections (erreurs critiques, incohérences, syntaxe ca65)*
...
| Texte           | 32×32 caractères, glyphes 4×4 px                                        |
...
| **✅ CORRECTION : **Les zones texte ($4800), attributs ($4C00) et sprites ($5000) sont des alias dans la plage VRAM ($4000–$5FFF). En mode texte, le VPU lit $4800/$4C00. En mode graphique, le VPU lit $4000 comme framebuffer. Ces zones sont physiquement les mêmes octets, mais leur interprétation est commandée par VPU_CTRL bit 0. |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
$4000  ├─────────────────────────────────────────────────────┤
       ...
       │  │  $4800–$4BFF  Mémoire texte 32×32 chars    │  │
       ...
       │  ┌─ COMMUN (alias selon mode) ─────────────────┐  │

       │  │  $6000–$6FFF  Tile/Charset ROM (VPU)        │  │
...
$F000  ├─────────────────────────────────────────────────────┤
       ...
       │  $F000–$F06F  API Jump Table (37 entrées × 3 oct.) │

       │  $F070–$F07F  Réservé extensions API              │
...
| -------------------- | ------------------------------------------- | --------------------- |
| $4000–$4FFF          | Tilemap $4800 + Attributs $4C00 (sous-zone) | Framebuffer A (début) |
| $5000–$5FFF          | Sprite Data (8 sprites × 256 oct.)          | Framebuffer A (fin)   |
| $6000–$6FFF          | Tile/Charset ROM (lu par VPU, alias ROM)    | Framebuffer B (début) |
...
| **Paramètre**         | **Valeur**                                      |
| --------------------- | ----------------------------------------------- |
| Résolution logique    | 32 colonnes × 32 lignes = 1 024 caractères      |
| Taille écran physique | 128×128 pixels                                  |
| Taille d'un caractère | 4×4 pixels (4 octets/char, 1 bit/pixel)         |
| Mémoire texte         | $4800–$4BFF (1 024 octets)                      |
| Attributs couleur     | $4C00–$4FFF (1 024 octets)                      |
| Charset               | $6000–$6FFF (4 096 octets) — alias Tile ROM VPU |

| **✅ CORRECTION : **v1.0 proposait deux formats charset contradictoires (16 oct/char et 4 oct/char). CORRIGÉ : format unique = 4 octets/char, 1 bit/pixel, grille 4×4 pixels. 256 chars × 4 oct = 1 024 octets en zone active ; la zone $6000–$6FFF (4 Ko) laisse de la place pour des charsets custom 8×8 en mode graphique. |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

Format charset (Mode texte) :

  1 caractère = 4 octets, 1 bit = 1 pixel (bi-level), grille 4×4

  Octet 0 : ligne 0 — bits 7-4 = pixels col 0-3 (bit7=col0)
...
$D00F  VPU_CHAR_OUT  Écriture : affiche char à position curseur + avance curseur
                    Newline ($0A) = retour chariot. Wrap automatique col 31 → 0.
...
$D220  MOUSE_X       Position X (0–127 gfx, 0–31 texte)
...
| **⛔ ERREUR CORRIGÉE : **v1.0 : le chapitre 3 déclarait $F000–$F7FF comme 'API Jump Table' entière. CORRIGÉ : $F000–$F07F = jump table (37 entrées × 3 oct = 111 oct), $F080–$F7FF = corps des routines ROM. |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| **⛔ ERREUR CORRIGÉE : **v1.0 SYS_SET_COLOR : la description disait INK=bits 7-4 mais l'attribut texte ($4C00) stocke PAPER=bits 7-4 et INK=bits 3-0. CORRIGÉ : SYS_SET_COLOR reçoit A avec INK=bits 7-4, PAPER=bits 3-0 (convention API). La routine écrit en RAM attribut avec les bits inversés (PAPER 7-4, INK 3-0). |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
Exemple d'usage :
  ...
  LDA #30    ; durée dans $80 : 30 frames
...
| $4800–$4BFF | Mémoire texte 32×32 (alias dans $4000–$5FFF)             |
| $4C00–$4FFF | Attributs couleur texte (alias dans $4000–$5FFF)         |
| $5000–$5FFF | Sprite Data (alias dans $4000–$5FFF)                     |
| $6000–$7FFF | Framebuffer B (mode gfx) / Tile+Charset ROM (mode texte) |
...
| $F000–$F06F | Jump table API (37 entrées × 3 oct)                      |

...
impl IoState {
    ...
    pub fn read_register(&mut self, addr: u16) -> u8 {
        match addr {
            ...
            // Système
            SYS_TIMER_LO => {
                self.timer_latch = (self.timer & 0xFF) as u8; // verrouille LO
                self.timer_latch
            }
            SYS_TIMER_HI => {
                (self.timer >> 8) as u8 // retourne HI sans reset
            }
            SYS_FRAME_LO => (self.frame_count & 0xFF) as u8,
            SYS_FRAME_HI => (self.frame_count >> 8) as u8,
    ...
    pub fn write_register(&mut self, addr: u16, val: u8) -> Option<IoAction> {
        match addr {
            ...
            VPU_SCROLL_X => {
                self.vpu.scroll_x = val;
                None
            }
            VPU_SCROLL_Y => {
                self.vpu.scroll_y = val;
                None
            }
            VPU_CURSOR_X => {
                self.vpu.cursor_x = val;
                None
            }
            VPU_CURSOR_Y => {
                self.vpu.cursor_y = val;
                None
            }
            VPU_INK => {
                self.vpu.ink = val & 0x0F;
                None
            }
            VPU_PAPER => {
                self.vpu.paper = val & 0x0F;
                None
            }
            ...
            // SPU
            SPU_MASTER_VOL => {
                self.spu.master_vol = val & 0x0F;
                None
            }
            0xD100..=0xD117 => {
                self.write_spu_voice(addr, val);
                None
            }
...
mod tests {
    ...
    fn key_set_and_clear() {
        ...
        assert_eq!(
            io.read_register(KEY_STATUS) & KEY_STATUS_PRESSED,
            KEY_STATUS_PRESSED
        );

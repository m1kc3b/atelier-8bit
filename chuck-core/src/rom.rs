...
pub fn build_rom() -> [u8; ROM_SIZE] {
    ...
    write_at(
        ...
        &[
            0xA5,
            0x80, // LDA (on utilise l'ABI — mais A est déjà le param)
    ...
    write_at(
        ...
        &[
            ...
            0x01, // AND #$01
            0xD0,
            0x0E, // BNE pixel_impair (+14)
            // px pair → nibble haut
            0xB1,
            0xF0, // LDA ($F0),Y (octet actuel)
            0x29,
            0x0F, // AND #$0F    efface nibble haut
            0xA6,
            0x00, // LDX $00     couleur
            0x8A, // TXA
            0x0A,
            0x0A,
            0x0A,
            0x0A, // ASL×4  couleur << 4
            0x05,
            0x03 + 4, // ORA... (simplifié — on utilise le ZP)
    ...
    write_at(
        ...
        &[
            ...
            0xAD, 0x00, 0xD2, // LDA $D200 (KEY_ASCII)
            0xA9, 0x00, 0x8D, 0x01, 0xD2, // STA $D201 (acquitter)
            0x60, // RTS
        ],
    );
    ...
    write_at(
        ...
        &[
            ...
            0xE0, 0x00, // CPX #$00
            0xF0, 0x09, // BEQ voix0
            0xE0, 0x01, 0xF0, 0x06, // BEQ voix1
            0x8D, 0x17, 0xD1, // STA $D117 (voix 2)
            0x60, 0x8D, 0x07, 0xD1, // STA $D107 (voix 0)
            0x60, 0x8D, 0x0F, 0xD1, // STA $D10F (voix 1)
            0x60,
        ],
    );
...
mod tests {
    ...
    fn jump_table_has_jmp_opcodes() {
        ...
        assert_eq!(
            rom[0x0000], 0x4C,
            "Premier octet de la ROM doit être JMP ($4C)"
        );
        // $F003 = SYS_DRAW_PIXEL
    ...
    fn charset_in_rom() {
        ...
        assert!(
            a_data.iter().any(|&b| b != 0),
            "Charset 'A' ne doit pas être tout zéro"
        );
    }
    ...
    fn all_jump_entries_valid() {
        ...
        for addr in entries {
            ...
            assert_eq!(
                rom[off], 0x4C,
                "Entry ${:04X} devrait être JMP ($4C), trouvé ${:02X}",
                addr, rom[off]
            );
        }
    }
